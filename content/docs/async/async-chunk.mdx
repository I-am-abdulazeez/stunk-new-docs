...
| `refresh.staleTime`            | `number`                   | `0`      | ms before data is considered stale                     |
| `refresh.cacheTime`            | `number`                   | `300000` | ms to keep cache after last subscriber leaves          |
| `refresh.refetchInterval`      | `number`                   | —        | Auto-refetch interval in ms                            |
| `refresh.refetchOnWindowFocus` | `boolean`                  | `false`  | Refetch when window regains focus                      |
...
postsChunk.setParams({ category: null }); // removes category
```
...
userChunk.reload();
userChunk.reload(); // returns the same in-flight promise
```
...
When params change, the default behavior is to show `null` while loading. Set `keepPreviousData: true` to keep showing the previous data instead — no flash of empty state:
...
// While loading new category, isPlaceholderData is true and data still shows old posts
postsChunk.setParams({ category: "engineering" });
```

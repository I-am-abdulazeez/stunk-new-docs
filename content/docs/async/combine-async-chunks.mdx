...
| --------- | ----------------------------- | --------------------------------------------- |
| `loading` | `boolean`                     | `true` if **any** source chunk is loading     |
| `error`   | `Error \| null`               | The first error encountered across all chunks |
| `errors`  | `Partial<Record<key, Error>>` | Per-chunk errors, keyed by name               |
| `data`    | `Record<key, T \| null>`      | Per-chunk data, keyed by name                 |

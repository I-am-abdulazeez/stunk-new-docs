...
| `scoped`               | `boolean`                  | `false` | Give each consumer its own isolated instance — see [Scoped Chunks](/docs/async/async-chunk#scoped-chunks) |
...
| `cacheTime`            | `number`                   | —       | ms to keep cache after last subscriber leaves — only evicted once no subscribers remain                   |

...
`persist` uses `localStorage` by default which is only available in browsers. In SSR environments, `persist` detects this automatically, logs a warning, and returns the base chunk unchanged — no crash, no side effects.

<Callout type="warn">
  If you need persistence in SSR, pass a custom `storage` implementation that
  works server-side, or guard the call with `typeof window !== 'undefined'`.
</Callout>

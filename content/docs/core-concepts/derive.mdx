...
Derive your data **outside** the component, then pass it to `useChunk()`:
...
function DiscountedPrice() {
  const [value] = useChunk(discounted); // just use useChunk
...
<Callout type="warn">
  ...
  deriving outside the component and passing to `useChunk()`. Start using the
  pattern above now.
</Callout>

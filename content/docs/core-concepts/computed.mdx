...
Use `useComputed` to consume computed state in a component:
...
function OrderSummary() {
  const total = useComputed([price, quantity], (p, q) => p * q);

  return <p>Total: ${total}</p>;
}
```

The component only re-renders when the computed result changes — not every time a dependency updates.
